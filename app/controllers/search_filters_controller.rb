class SearchFiltersController < ApplicationController
  def create
    ...
    if !params[:search] || params[:search] == t('products.left_column.search')
      redirect_to products_path(:package => params[:package], :list_id => params[:list_id], :category_id => params[:category_id], :actor_id => params[:actor_id], :director_id => params[:director_id], :studio_id => params[:studio_id], :filter => params[:filter])
    else
