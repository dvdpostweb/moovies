      ...
      if discount.present?
        ...
        if customer.save(validate: false)
          ...
          if activation_code.save(validate: false)
            orange_is_eligable_wcf_service = HTTParty.get("https://www.plush.be:2355/WcfService/http/OrangeIsEligable?customersId=#{customer.customers_id}&mobileNumber=#{params[:sms_number]}&SMSCodeMessage=#{puts t("orange.sms_code.message")}#{activation_code.sms_authentification_code}&products_id=#{product_id_from_params}&locale=#{set_locale(I18n.locale)}")
  ...
  def orange_purchase
    if request.xhr?
      ...
      if sms_code.present?
        customer = Customer.find(sms_code.customers_id)
        if customer.present?
          orange_purchase_wcf_service = HTTParty.get("https://www.plush.be:2355/wcfservice/http/OrangePurchase?customersId=#{customer.customers_id}&mobileNumber=#{params[:plush_phone_number]}&price=0&products_id=#{product_id_from_params}&message=subscription&payment_id=0&locale=#{set_locale(I18n.locale)}")
    ...
    if request.xhr?
      ...
      if sms_code.present?
        ...
        if customer.present?
          ...
          customer.customers_abo = 1
          if customer.save(validate: false)
            orange_purchase_wcf_service = HTTParty.get("https://www.plush.be:2355/wcfservice/http/OrangePurchase?customersId=#{customer.customers_id}&mobileNumber=#{params[:plush_phone_number]}&price=0&products_id=#{product_id_from_params}&message=ppv2&payment_id=0&locale=#{set_locale(I18n.locale)}")
            if orange_purchase_wcf_service.parsed_response == "TRUE"
  ...
  def orange_purchase_step3
    if request.xhr?
      ...
      if sms_code.present?
        ...
        if customer.present?
          orange_purchase_wcf_service = HTTParty.get("https://www.plush.be:2355/wcfservice/http/OrangePurchase?customersId=#{customer.customers_id}&mobileNumber=#{params[:plush_phone_number]}&price=0&products_id=#{customer.customers_abo_type}&message=ppv2&payment_id=0&locale=#{set_locale(I18n.locale)}")
          if orange_purchase_wcf_service.parsed_response == "TRUE"
              customer.customers_registration_step = 100
              customer.customers_abo = 1
              customer.customers_abo_validityto = Date.today + 1.month
