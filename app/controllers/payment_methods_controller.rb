class PaymentMethodsController < ApplicationController
  ...
  def update
    ...
    OgoneCheck.create(:orderid => @order_id, :amount => (@price*100).round, :customers_id => current_customer.to_param, :context => internal_com, :site => 1, :language_id => Moovies.customer_languages[I18n.locale], :products_id => product_id)
    ...
    #  @hash = Digest::SHA1.hexdigest("#{@order_id}#{(@price*100).round}EUR#{Moovies.ogone_pspid[Rails.env]}#{@alias}#{@com}#{Moovies.ogone_pass[Rails.env]}")
    #else
