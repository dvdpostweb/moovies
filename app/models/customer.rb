class Customer < ActiveRecord::Base
  ...
  def get_all_tokens(kind = nil, type = nil, page = 1)
    if type == :old
      tokens.expired(48.hours.ago.localtime).select('distinct vod_wishlists.*').ordered_old.joins(:products).where(:products => {:products_type => Moovies.product_kinds[kind]}).paginate(:per_page => 20, :page => page)
    else
      if !kind.nil?
