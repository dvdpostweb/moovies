...
class Customer < ActiveRecord::Base
  ...
  def email_abo
    errors.add(:email, I18n.t("errors.messages.taken")) if Customer.where(:email => self.email, :customers_abo => 1).where_not(:customers_abo_type => 6).exists? && (@activation && !@activation.all_cust? || @activation.nil?)
  end
