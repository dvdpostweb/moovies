ThinkingSphinx::Index.define :product, :with => :active_record, :name => 'product_nl' do
  ...
  has "if(vod_online_nls_products.expire_at >= date(now()),vod_online_nls_products.expire_at,vod_online_nls_products.expire_backcatalogue_at)", :type => :timestamp, :as => :streaming_expire_at
  ...
  has "concat(GROUP_CONCAT(DISTINCT IFNULL(`vod_online_nls_products`.`language_id`, '0') SEPARATOR ','),',', GROUP_CONCAT(DISTINCT IFNULL(`vod_online_nls_products`.`subtitle_id`, '0') SEPARATOR ','))", :type => :integer, :multi => true, :as => :audio_sub
