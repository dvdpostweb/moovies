ThinkingSphinx::Index.define :product, :with => :active_record, :name => 'product_be' do
  ...
  has "ifnull(concat('2',replace(if(max(vod_online_bes_products.expire_at) > date(now()),max(vod_online_bes_products.available_from),null),'-','')),ifnull(concat('2',replace(max(vod_online_bes_products.available_backcatalogue_from), '-','')), ifnull(concat('1',replace(if(max(streaming_products.expire_at) > date(now()),max(streaming_products.available_from),null), '-','')),concat('1',replace(max(streaming_products.available_backcatalogue_from), '-','')))))", :type => :integer, :as => :streaming_available_at_order
  has "ifnull(if(max(vod_online_bes_products.expire_at) > date(now()),max(vod_online_bes_products.available_from),null),ifnull(max(vod_online_bes_products.available_backcatalogue_from), ifnull(if(max(streaming_products.expire_at) > date(now()),max(streaming_products.available_from),null),max(streaming_products.available_backcatalogue_from))))", :type => :timestamp, :as => :streaming_available_at
