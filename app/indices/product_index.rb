ThinkingSphinx::Index.define :product, :with => :active_record, :name => 'product_be' do
  ...
  has "(select if( date(now()) <= date(min(expire_at)) ,min(available_from), if(date(now()) <= date(max(expire_at)), max(available_from),min(available_backcatalogue_from)))  from streaming_products   where status = 'online_test_ok' and available = 1  and imdb_id = products.imdb_id and   (date(now())  > expire_at or date(now()) <= date(expire_at) or expire_at is null) order by available_from asc, available_backcatalogue_from asc limit 1)", :type => :integer, :as => :streaming_available_at_order
